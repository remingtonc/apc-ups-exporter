...
func main() {
	...
	flag.StringVar(&flagNisAddress, "nis-address", flagNisAddress, "The hostname or IP address of the apcupsd Network Information Server.")
	flag.IntVar(&flagNisPort, "nis-port", flagNisPort, "The port number of the apcupsd Network Information Server.")
	flag.StringVar(&flagMetricsAddress, "metrics-address", flagMetricsAddress, "The hostname or IP address to listen on for the Prometheus HTTP metrics server.")
	...
	flag.Usage = func() {
		...
		fmt.Printf("\nUsage: %s [-h/-help] [-nis-address <hostname or IP>] [-nis-port <number>] [-metrics-address <hostname or IP>] [-metrics-port <number>] [-metrics-path <string>] [-metrics-interval <seconds>]\n", os.Args[0])
	...
	// Require a host for the Network Information Server
	if flagNisAddress == "" {
		exitWithErrorMessage("Invalid hostname or IP address for apcupsd's Network Information Server.")
	}

	// Require a valid port number for the Network Information Server
	if flagNisPort <= 0 || flagNisPort >= 65536 {
		exitWithErrorMessage("Invalid port number for apcupsd's Network Information Server.")
	}

	// Require a host for the Prometheus HTTP metrics server
	if flagMetricsAddress == "" {
		exitWithErrorMessage("Invalid listening hostname or IP address for the Prometheus HTTP metrics server.")
	}

	// Require a valid port number for the Prometheus HTTP metrics server
	if flagMetricsPort <= 0 || flagMetricsPort >= 65536 {
		exitWithErrorMessage("Invalid listening port number for the Prometheus HTTP metrics server.")
	}
	...
	go collectMetricsInBackground(flagMetricsInterval, flagNisAddress, flagNisPort)

	// Serve the metrics page
	fmt.Printf("Serving metrics page at http://%s:%d%s...\n", flagMetricsAddress, flagMetricsPort, flagMetricsPath)
	ServeMetrics(flagMetricsAddress, flagMetricsPort, flagMetricsPath)

}

// Runs in the background to periodically collect metrics
func collectMetricsInBackground(interval int, nisAddress string, nisPort int) {
...
func updateMetrics(nisAddress string, nisPort int) {
	...
	if statusError != nil {
		exitWithErrorMessage(statusError.Error())
	}
